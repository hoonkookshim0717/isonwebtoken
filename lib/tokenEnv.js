...
class TokenEnv {
	...
	constructor(options) {
		if(!options || typeof options !== 'object') throw new Error(ERRORS.apps.OPTION_NOT_PROVIDED);
...
console.log("this.envSettings: ", this.envSettings);
	...
	set(propertyName, customFns) {
		if(typeof propertyName === 'function') {
			try {
				...
				throw new TypeError(ERRORS.apps.INVALID_RETURN_OBJ);
			}
			return this;
		}
		...
		} else throw new TypeError(ERRORS.apps.INVALID_ARGS_FOR_SET);
		return this;
	}

	setUserCode(code, thing) {
		// Type checking.
		if(typeof code !== 'string') throw new TypeError(ERRORS.apps.INVALID_ARG_FOR_SET_USER_CODE);
		if(!MEN.isBase64UrlString.test(code)) throw new TypeError(ERRORS.apps.INVALID_ARG_FOR_SET_USER_CODE);
		if(this.userRegDecodeTable.has(code)) throw new Error(ERRORS.apps.USER_CODE_ALREADY_REGISTERED + code);
